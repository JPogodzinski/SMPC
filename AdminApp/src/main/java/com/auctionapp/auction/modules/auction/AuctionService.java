...
public class AuctionService {
    ...
    private void millionaireProblem(Bidder firstBidder, Bidder secondBidder) {
        ...
        System.out.println("PETLA FOR =================================");      // todo delete
        ...
        for (BigInteger j = BigInteger.valueOf(0); j.compareTo(MAX) < 0; j = j.add(BigInteger.ONE))
        {
            ...
            Y.add(modpow);
            //System.out.println(j.add(BigInteger.ONE) + " value of Y[" + j + "]:  " + Y.get(j.intValue()));  // todo delete
        }
        // if j = J, Alicia should get value of X

        //=============================
        // Alicia picks P which length is N/2 (at least close to N/2), where N is the bit-length of X
        // P is prime number
        // appended values in list Z[] must differ by at least 2
        // =============================

        ArrayList<BigInteger> Z = new ArrayList<BigInteger>();
        System.out.println("X's bit length is:  " + X.bitLength());     // todo delete
        BigInteger P;
        ...
        {
            System.out.println("Bob bidded more (or same)");// todo delete
